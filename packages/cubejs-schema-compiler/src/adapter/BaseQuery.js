...
export class BaseQuery {
  ...
  outerMeasuresJoinFullKeyQueryAggregate(innerMembers, outerMembers, toJoin) {
    ...
    return `SELECT ${this.topLimit()}${columnsToSelect} FROM ${this.wrapInParenthesis(toJoin[0])} ${this.asSyntaxTable} q_0 ${join}${havingFilters}${this.orderBy()}${this.groupByDimensionLimit()}`;
  }
